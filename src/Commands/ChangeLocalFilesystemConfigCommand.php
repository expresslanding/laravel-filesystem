...
class ChangeLocalFilesystemConfigCommand extends FilesystemCommand
{
     ...
     * @var string
     */
    protected $signature = 'filesystem:config:local {--name=} {--root=} {--url=} {--visibility=} {--throw=}';
     ...
     * @var string
     */
    protected $description = 'Change local disk config. Parameters:
    ...
    public function handle(): bool
    {
        ...
        try {
            $filesystem = (new Filesystem())->changeLocalDiskConfig($this->option('name'), $config);
            $body       = [
                $filesystem->id,
                $filesystem->name,
                $filesystem->size,
            ...
            $this->info(sprintf('New config for "%s" has been apply ', $filesystem->name));
